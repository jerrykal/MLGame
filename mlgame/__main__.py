...
if __name__ == "__main__":
    ...
    path_of_ai_clients = revise_ai_clients(
        arg_obj.ai_clients, game_config.user_num_config
    )
    ...
    from mlgame.core.process import (
        ...
        create_process_of_progress_log_and_start,
        create_process_of_ws_and_start,
        terminate,
    )
    from mlgame.view.view import DummyPygameView, PygameView

    game_comm = GameCommManager(wait_action=arg_obj.wait_action)
    try:
        if arg_obj.ws_url:
        ...
        if arg_obj.is_manual:
            ...
            game_executor = GameExecutor(
                ...
                wait_action=arg_obj.wait_action,
            )
        time.sleep(0.1)
        game_executor.run()
    ...
    print(
        f"===========All process is terminated at {datetime.datetime.now()}==========="
    )
    pass
