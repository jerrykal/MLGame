...
def create_cli_args_parser():
    ...
    description_str = (
        "A platform for applying machine learning algorithm "
        "to play pixel games. "
        "In default, the game runs in the machine learning mode. "
    )

    parser = ArgumentParser(
        usage=usage_str, description=description_str, add_help=False
    )
    ...
    group.add_argument("--ws_url", type=str, dest="ws_url", help="ws_url route")
    ...
    group.add_argument(
        ...
        dest="wait_action",
        default=False,
        help="wait for the action from the user at each frame. [default: %(default)s]",
    )
